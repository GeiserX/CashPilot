...
class TestRedeployTakesTheCatalogCommand:
    ...
    CREDENTIAL_RECORD = {
        "image": "honeygain/honeygain:latest",
        "env": {"HG_EMAIL": "me@example.com", "HG_PASSWORD": "old-secret-4412"},
        "command": "-tou-accept -email me@example.com -pass old-secret-4412",
    }
    ...
    def test_a_blank_redeploy_fills_the_command_from_the_recorded_credentials(self, client):
        ...
        assert sent["command"] == "-tou-accept -email me@example.com -pass old-secret-4412"
    ...
    def test_a_retyped_password_reaches_the_command_line(self, client):
        data, sent = self._redeploy(
            client, self.CREDENTIAL_SVC, self.CREDENTIAL_RECORD, {"env": {"HG_PASSWORD": "new-secret-9051"}}
        )
        assert sent["command"] == "-tou-accept -email me@example.com -pass new-secret-9051"

    def test_the_report_never_repeats_the_command(self, client):
        """Commands carry credentials; the toast and the log line must not."""
        data, _ = self._redeploy(
            client, self.CREDENTIAL_SVC, self.CREDENTIAL_RECORD, {"env": {"HG_PASSWORD": "new-secret-9051"}}
        )
        report = " ".join(data["kept_from_previous_deployment"])
        assert "command:" in report
        for secret in ("new-secret-9051", "old-secret-4412", "me@example.com"):
